...
def calculateOpticFlowData(frameNumberX, frameNumberY,frameTime, frameToSample = 0, gridOffset = 5, frameScaleFactor = 3, writeFrames = True, isNormalizedVector = True, vectorScaleFactor = 7, showZeroVectors = False, filterType = 0, filterScaleX = 1, filterScaleY = 1, filterScaleT = 1, compareResults = False, magnitudeConstraint = 20, view=True):
    ...
    while(ret):
        ...
        colorIterator = 0
        opticFlow = OpticFlow(frameInitial_blur, frameDelta_blur,cornerList,filterType,filterScaleX,filterScaleY,filterScaleT)
        
        if(compareResults):
            p0 = cv2.goodFeaturesToTrack(frameInitial_blur,100, mask = None,)
            p1, st, err = cv2.calcOpticalFlowPyrLK(frameInitial_blur,frameDelta_blur,p0, None)
        ...
        for corners in cornerList:
            ...
            if(x < frameDeltaResize.shape[0] and y < frameDeltaResize.shape[1]):
                if(isNormalizedVector):
                    ...
                    normalizedVector = (int(opticFlow[0][x,y]  ), int(opticFlow[1][x,y]))
                                                
                if((x,y) != (x - normalizedVector[0],y + normalizedVector[1]) or showZeroVectors):
                    
                    resultantMagnitude = calculateMagnitude((x,y), ( x - int(opticFlow[0][x,y]  ),y - int(opticFlow[1][x,y])))
                    ...
                    if(resultantMagnitude > magnitudeConstraint or resultantMagnitude == 0 and view):
                        mask = cv2.arrowedLine(mask,(x,y), (int(x + (normalizedVector[0]*vectorScaleFactor)),int(y + (normalizedVector[1]*vectorScaleFactor)),),color[colorIterator].tolist(),1, tipLength=.25)
            if(view):
        ...
        if(writeFrames):
            filePath = "sampleFrames/frames" + str(int(isNormalizedVector)) + str(vectorScaleFactor) + str(filterType) + str(filterScaleX).replace(".","-") + str(filterScaleY).replace(".","-")+ str(filterScaleT).replace(".","-")+"FrameX-" + str(frameNumberX)+ "FrameY" + str(frameNumberY)+ str(magnitudeConstraint) + "/"
    ...
    fileName = "dataFiles/AssayOutput" + str(int(isNormalizedVector)) + str(vectorScaleFactor) + str(filterType) + str(filterScaleX).replace(".","-") + str(filterScaleY).replace(".","-")+ str(filterScaleT).replace(".","-")+ str(magnitudeConstraint) + ".txt"
