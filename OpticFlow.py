...
def OpticFlow(frame1, frame2, cornerList):
    ...
    frameX = cv2.filter2D(frame2, -1, convolveFilterX) + cv2.filter2D(frame1, -1, convolveFilterX) 
    frameY = cv2.filter2D(frame2, -1, convolveFilterY) + cv2.filter2D(frame1, -1, convolveFilterY)
    frameT =  cv2.filter2D(frame1, -1, convolveFilterTi) - cv2.filter2D(frame2, -1, convolveFilterT)
...
while(ret):
    ...
    for corners in cornerList:
        ...
        if(x < frameDeltaResize.shape[0] and y < frameDeltaResize.shape[1]):
            ...
            if((x,y) != (x + normalizedVector[0],y + normalizedVector[1])):
                ...
                print()
                mask = cv2.arrowedLine(mask,(x,y), (int(x - (normalizedVector[0]*vectorScaleFactor)),int(y - (normalizedVector[1]*vectorScaleFactor))),color[i].tolist(),1, tipLength=.25)
            if(i % 2 == 0 ):
                mask = np.zeros_like(frameInitialResize)
            print(i % 100)
    ...
    cornerList = cv2.goodFeaturesToTrack(frameDeltaGrey, 100, 0.01, 5)